...
if (isset($_GET['id']) && isset($_GET['cod'])) {
    ...
    $stm = $pdo->prepare("SELECT * FROM citas_Seguimientos_Contratos WHERE cod_Cita_Est =  $cod_Est AND cod_Cita_Cont = $cod_P");
    $stm->execute();
...
if ($_POST) {
    ...
    try {
        ...
        $stm = $pdo->prepare('CALL ActualizarCitaContrato(?,?,?,?,?,?,?)');
